    ...
    try {
      ...
      const result = await safeFetchJson<EmailAnalysis>('/api/ai/analyze', {
        ...
        body: JSON.stringify({
          ...
          bodyText: emailBodyForAI(email.bodyText || email.snippet, config.provider, settings.allowCloudEmailBody),
